...
def collect_driver_statistics(
    ...
    *,
    ...
    if enforced_MoI is None:
        raise ValueError(
            "enforced_MoI is required: graphs are constructed at a stated "
            "Margin of Insecurity (see the results table in the writeup)."
        )
    if isinstance(enforced_MoI, bool) or not isinstance(enforced_MoI, int):
        raise TypeError("enforced_MoI must be an int.")
    if enforced_MoI < 0:
