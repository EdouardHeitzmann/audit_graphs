...
import src.experiments.driver_statistics as experiment
...
def test_collect_driver_statistics_runs_unseeded_noise_and_delta_trials(
    monkeypatch,
    capsys,
):
    ...
    monkeypatch.setattr(experiment, "heap_based_search", fake_heap_based_search)
    ...
    assert search_kwargs == [
        {
            ...
            "m": 2,
            "memory_lite": True,
            "constructor_cls": experiment.WIGMGraphConstructor,
            "simultaneous": True,
            "verify_output": True,
        }
    ]
...
def test_collect_driver_statistics_can_use_an_enforced_moi(
    ...
    monkeypatch.setattr(
        experiment,
        "heap_based_search",
        lambda **kwargs: pytest.fail("Heap search should not be used."),
    )
    monkeypatch.setattr(
        experiment,
...
def test_loaded_profile_requires_number_of_seats():
    ...
    with pytest.raises(ValueError, match="m is required"):
        experiment.collect_driver_statistics(profile, verbose=False)
